...
class ImageMatcherFLANN:
    def __init__(self, sample_images_folder,gg):
    ...
    def load_incoming_image(self, incoming_image_path='/content/drive/MyDrive/SIH/lalpath_invoice_bill5_page_1.jpg'):
...
def runner3(incoming_image_path,sample_images_folder):
    image_matcher = ImageMatcherFLANN(sample_images_folder,incoming_image_path)
    similar_images_with_scores = image_matcher.find_similar_images()
    res = {}
    for index, (image_path, score) in enumerate(similar_images_with_scores, start=1):
        # print(f"Top {index} Similar Image Path: {image_path}, Score: {score}")
